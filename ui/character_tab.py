...
def build_character_tab(self):
    ...
    load_btn = ctk.CTkButton(self.character_tab, text="Load character_state.txt", command=self.load_character_state, font=("latin modern roman", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.character_wordcount_label = ctk.CTkLabel(self.character_tab, text="Word Count: 0", font=("latin modern roman", 12))
    self.character_wordcount_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.character_tab, text="Save Changes", command=self.save_character_state, font=("latin modern roman", 12))
    save_btn.grid(row=0, column=2, padx=5, pady=5, sticky="e")

    self.character_text = ctk.CTkTextbox(self.character_tab, wrap="word", font=("latin modern roman", 12))
...
def save_character_state(self):
    ...
    self.log("Changes to character_state.txt have been saved.")
