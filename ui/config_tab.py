...
def create_label_with_help(self, parent, label_text, tooltip_key, row, column,
    ...
    """
    Encapsulates a Label with a "?" button to display help information.
    """
...
def build_config_tabview(self):
    """
    Create tabs for LLM Model settings and Embedding settings.
    """
    ...
    # Save and Load config buttons at the bottom
    ...
    save_config_btn = ctk.CTkButton(self.btn_frame_config, text="Save current interface configuration to file", command=self.save_config_btn, font=("Latin Modern Roman", 12))
    save_config_btn.grid(row=0, column=0, padx=5, pady=5, sticky="ew")

    load_config_btn = ctk.CTkButton(self.btn_frame_config, text="Load current interface configuration into the program", command=self.load_config_btn, font=("Latin Modern Roman", 12))
...
def build_ai_config_tab(self):
    ...
    # 1) API Key
    create_label_with_help(self, parent=self.ai_config_tab, label_text="LLM API Key:", tooltip_key="api_key", row=0, column=0, font=("Latin Modern Roman", 12))
    api_key_entry = ctk.CTkEntry(self.ai_config_tab, textvariable=self.api_key_var, font=("Latin Modern Roman", 12),show="*")
    api_key_entry.grid(row=0, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    # 2) Base URL
    create_label_with_help(self, parent=self.ai_config_tab, label_text="LLM Base URL:", tooltip_key="base_url", row=1, column=0, font=("Latin Modern Roman", 12))
    base_url_entry = ctk.CTkEntry(self.ai_config_tab, textvariable=self.base_url_var, font=("Latin Modern Roman", 12))
    base_url_entry.grid(row=1, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    # 3) Interface format
    create_label_with_help(self, parent=self.ai_config_tab, label_text="LLM Interface Format:", tooltip_key="interface_format", row=2, column=0, font=("Latin Modern Roman", 12))
    interface_options = ["DeepSeek", "Aliyun Bailian", "OpenAI", "Azure OpenAI", "Azure AI", "Ollama", "ML Studio", "Gemini", "Volcano Engine", "SiliconFlow"]
    interface_dropdown = ctk.CTkOptionMenu(self.ai_config_tab, values=interface_options, variable=self.interface_format_var, command=on_interface_format_changed, font=("Latin Modern Roman", 12))
    interface_dropdown.grid(row=2, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    # 4) Model Name
    create_label_with_help(self, parent=self.ai_config_tab, label_text="Model Name:", tooltip_key="model_name", row=3, column=0, font=("Latin Modern Roman", 12))
    model_name_entry = ctk.CTkEntry(self.ai_config_tab, textvariable=self.model_name_var, font=("Latin Modern Roman", 12))
    model_name_entry.grid(row=3, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    # 5) Temperature
    create_label_with_help(self, parent=self.ai_config_tab, label_text="Temperature:", tooltip_key="temperature", row=4, column=0, font=("Latin Modern Roman", 12))
    ...
    self.temp_value_label = ctk.CTkLabel(self.ai_config_tab, text=f"{self.temperature_var.get():.2f}", font=("Latin Modern Roman", 12))
    self.temp_value_label.grid(row=4, column=2, padx=5, pady=5, sticky="w")

    # 6) Max Tokens
    create_label_with_help(self, parent=self.ai_config_tab, label_text="Max Tokens:", tooltip_key="max_tokens", row=5, column=0, font=("Latin Modern Roman", 12))
    ...
    self.max_tokens_value_label = ctk.CTkLabel(self.ai_config_tab, text=str(self.max_tokens_var.get()), font=("Latin Modern Roman", 12))
    self.max_tokens_value_label.grid(row=5, column=2, padx=5, pady=5, sticky="w")

    # 7) Timeout (sec)
    create_label_with_help(self, parent=self.ai_config_tab, label_text="Timeout (sec):", tooltip_key="timeout", row=6, column=0, font=("Latin Modern Roman", 12))
    ...
    self.timeout_value_label = ctk.CTkLabel(self.ai_config_tab, text=str(self.timeout_var.get()), font=("Latin Modern Roman", 12))
    self.timeout_value_label.grid(row=6, column=2, padx=5, pady=5, sticky="w")

    # Add Test button
    test_btn = ctk.CTkButton(self.ai_config_tab, text="Test Configuration", command=self.test_llm_config, font=("Latin Modern Roman", 12))
...
def build_embeddings_config_tab(self):
    ...
    # 1) Embedding API Key
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text="Embedding API Key:", tooltip_key="embedding_api_key", row=0, column=0, font=("Latin Modern Roman", 12))
    emb_api_key_entry = ctk.CTkEntry(self.embeddings_config_tab, textvariable=self.embedding_api_key_var, font=("Latin Modern Roman", 12))
    emb_api_key_entry.grid(row=0, column=1, padx=5, pady=5, sticky="nsew")

    # 2) Embedding Interface Format
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text="Embedding Interface Format:", tooltip_key="embedding_interface_format", row=1, column=0, font=("Latin Modern Roman", 12))

    emb_interface_options = ["DeepSeek", "OpenAI", "Azure OpenAI", "Gemini", "Ollama", "ML Studio", "SiliconFlow"]

    emb_interface_dropdown = ctk.CTkOptionMenu(self.embeddings_config_tab, values=emb_interface_options, variable=self.embedding_interface_format_var, command=on_embedding_interface_changed, font=("Latin Modern Roman", 12))
    emb_interface_dropdown.grid(row=1, column=1, padx=5, pady=5, sticky="nsew")

    # 3) Embedding Base URL
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text="Embedding Base URL:", tooltip_key="embedding_url", row=2, column=0, font=("Latin Modern Roman", 12))
    emb_url_entry = ctk.CTkEntry(self.embeddings_config_tab, textvariable=self.embedding_url_var, font=("Latin Modern Roman", 12))
    emb_url_entry.grid(row=2, column=1, padx=5, pady=5, sticky="nsew")

    # 4) Embedding Model Name
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text="Embedding Model Name:", tooltip_key="embedding_model_name", row=3, column=0, font=("Latin Modern Roman", 12))
    emb_model_name_entry = ctk.CTkEntry(self.embeddings_config_tab, textvariable=self.embedding_model_name_var, font=("Latin Modern Roman", 12))
    emb_model_name_entry.grid(row=3, column=1, padx=5, pady=5, sticky="nsew")

    # 5) Retrieval Top-K
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text="Retrieval Top-K:", tooltip_key="embedding_retrieval_k", row=4, column=0, font=("Latin Modern Roman", 12))
    emb_retrieval_k_entry = ctk.CTkEntry(self.embeddings_config_tab, textvariable=self.embedding_retrieval_k_var, font=("Latin Modern Roman", 12))
    emb_retrieval_k_entry.grid(row=4, column=1, padx=5, pady=5, sticky="nsew")

    # Add Test button
    test_btn = ctk.CTkButton(self.embeddings_config_tab, text="Test Configuration", command=self.test_embedding_config, font=("Latin Modern Roman", 12))
...
def load_config_btn(self):
    ...
    if cfg:
        ...
        messagebox.showwarning("Warning", "Configuration file not found or unreadable.")
