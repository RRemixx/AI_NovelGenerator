...
def build_directory_tab(self):
    ...
    load_btn = ctk.CTkButton(self.directory_tab, text="Load Novel_directory.txt", command=self.load_chapter_blueprint, font=("latin modern roman", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.directory_word_count_label = ctk.CTkLabel(self.directory_tab, text="Word Count: 0", font=("latin modern roman", 12))
    self.directory_word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.directory_tab, text="Save Changes", command=self.save_chapter_blueprint, font=("latin modern roman", 12))
    save_btn.grid(row=0, column=2, padx=5, pady=5, sticky="e")

    self.directory_text = ctk.CTkTextbox(self.directory_tab, wrap="word", font=("latin modern roman", 12))
...
def load_chapter_blueprint(self):
    ...
    self.log("Loaded content of Novel_directory.txt into the editor.")
...
def save_chapter_blueprint(self):
    ...
    self.log("Changes to Novel_directory.txt have been saved.")
