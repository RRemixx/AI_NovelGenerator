...
def build_setting_tab(self):
    ...
    load_btn = ctk.CTkButton(self.setting_tab, text="Load Novel_architecture.txt", command=self.load_novel_architecture, font=("Latin Modern Roman", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.setting_word_count_label = ctk.CTkLabel(self.setting_tab, text="Word Count: 0", font=("Latin Modern Roman", 12))
    self.setting_word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.setting_tab, text="Save Changes", command=self.save_novel_architecture, font=("Latin Modern Roman", 12))
    save_btn.grid(row=0, column=2, padx=5, pady=5, sticky="e")

    self.setting_text = ctk.CTkTextbox(self.setting_tab, wrap="word", font=("Latin Modern Roman", 12))
...
def load_novel_architecture(self):
    ...
    if not filepath:
        messagebox.showwarning("Warning", "Please set the file save path first.")
        return
    ...
    self.log("Loaded content from Novel_architecture.txt into the editor.")
...
def save_novel_architecture(self):
    ...
    self.log("Changes to Novel_architecture.txt have been saved.")
