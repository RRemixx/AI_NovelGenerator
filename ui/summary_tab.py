...
def build_summary_tab(self):
    ...
    load_btn = ctk.CTkButton(self.summary_tab, text="Load global_summary.txt", command=self.load_global_summary, font=("Latin Modern Roman", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.word_count_label = ctk.CTkLabel(self.summary_tab, text="Word count: 0", font=("Latin Modern Roman", 12))
    self.word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.summary_tab, text="Save changes", command=self.save_global_summary, font=("Latin Modern Roman", 12))
    save_btn.grid(row=0, column=2, padx=5, pady=5, sticky="e")

    self.summary_text = ctk.CTkTextbox(self.summary_tab, wrap="word", font=("Latin Modern Roman", 12))
...
def save_global_summary(self):
    ...
    self.log("Changes to global_summary.txt have been saved.")
